...
- Drive the device by opening http://127.0.0.1:8080 (the WYSIWYG draw tool) or by pointing any BUSY Bar app at the host over the HTTP API. Apps are not bundled here; they live in the community gallery / busybar-manager and target the emulator exactly as they would real hardware (just swap the host).
...
A Node server and a Vue renderer, driven by external client apps over the device's HTTP API and a one-way SSE stream:

```
client apps ──POST /api/display/draw──▶  server.js  ──SSE /events──▶  browser (Vue) renders LEDs
(external)                               (mock API + in-memory state)
```
...
- **Font names** are the device's set: `tiny small normal condensed bold large extra_large global` (see `ATLAS_KEY` in `web/src/lib/atlas.js`). Colors are `#RRGGBBAA` strings.
